...
interface Props {
  goal: Goal;
  onSubmit: (updates: { name?: string; target_amount?: number; weekly_contribution?: number }) => Promise<void>;
  onClose: () => void;
}

export default function EditGoalModal({ goal, onSubmit, onClose }: Props) {
  const [name, setName] = useState(goal.name);
  const [targetAmount, setTargetAmount] = useState(goal.targetAmount.toString());
  const [weeklyContribution, setWeeklyContribution] = useState(goal.weeklyContribution.toString());
  ...
  const handleSubmit = async () => {
    ...
    const numWeekly = parseFloat(weeklyContribution);
        ...
        >
          ...
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Pencil className="h-5 w-5 text-lilac-500" />
              <h2 className="font-display text-lg font-semibold text-ink">Edit Goal</h2>
            </div>
            <button
              onClick={onClose}
